...
import { isLMStudioURL, lmStudio } from "./lm-studio";
...
export async function* chatStream(
  ...
  // Handle static/mock/replay modes
  if (
  ...
  if (isLMStudioURL(env.NEXT_PUBLIC_API_URL)) {
    console.log("🎯 Using LM Studio provider");
    return yield* lmStudio.chat(userMessage, options);
  }

  // Default: Use Deer-Flow backend
  console.log("🔄 Using Deer-Flow backend");
  
  try {
    const serviceURL = resolveServiceURL("chat/stream");
    ...
    const stream = fetchStream(serviceURL, {
      method: "POST",
      headers: {
    ...
    console.error("Deer-Flow backend error:", error);
    throw error;
  }
}
